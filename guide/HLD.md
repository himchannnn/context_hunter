...
### 6.5 Daily & Shop Module (일일/상점)
*   **역할**: 일일 미션 수행 여부 트래킹 및 보상 지급, 아이템(테마) 구매 관리
*   **관계**: `DailyProgress` 및 `User`(credits, owned_themes) 테이블 관리
...
| **Daily** | POST | `/api/daily-progress` | 일일 진행상황 업데이트 |
| **Shop** | POST | `/api/shop/buy` | 테마 구매 |
